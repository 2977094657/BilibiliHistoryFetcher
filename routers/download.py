...
try:
    ...
    print(
        "无法从 history 模块导入_process_image_url、get_video_by_cid 和_process_record 函数"
    )
...
async def stream_process_output(process: subprocess.Popen):
    ...
    try:
        ...
        # 读取可能的错误输出
        stderr_output = await asyncio.get_event_loop().run_in_executor(
...
async def download_video(request: DownloadRequest):
    ...
    try:
        ...
        if config["yutto"]["basic"]["login_strict"]:
            ...
            if not sessdata:
                raise HTTPException(
                    status_code=401, detail="未登录：当前设置要求必须登录才能下载视频"
                )
        ...
        if getattr(sys, "frozen", False):
            ...
            paths_to_try = [
                ...
                os.path.join(
                    base_path, "_internal", "yutto.exe"
                ),  # 尝试 _internal 目录
                os.path.join(os.getcwd(), "yutto.exe"),  # 尝试当前工作目录
                os.path.join(
                    os.getcwd(), "_internal", "yutto.exe"
                ),  # 尝试当前工作目录的 _internal
            ]
            ...
            if yutto_path is None:
                raise FileNotFoundError(
                    f"找不到 yutto.exe，已尝试的路径：{', '.join(paths_to_try)}"
                )
        else:
            # 如果是直接运行 python 脚本
            yutto_path = "yutto"
        ...
        if not os.access(download_dir, os.W_OK):
            raise HTTPException(
                status_code=500, detail=f"没有下载目录的写入权限：{download_dir}"
            )
        if not os.access(tmp_dir, os.W_OK):
            raise HTTPException(
                status_code=500, detail=f"没有临时目录的写入权限：{tmp_dir}"
            )
        ...
        command = [
            ...
            tmp_dir,
            "--subpath-template",
            f"{{title}}_{{username}}_{{download_date@%Y%m%d_%H%M%S}}_{request.cid}/{{title}}_{request.cid}",
        ...
        if request.only_audio:
            ...
            command.extend(["--acodec", "flac:copy"])
        ...
        if config["yutto"]["danmaku"]["font_size"]:
            command.extend(
                ["--danmaku-font-size", str(config["yutto"]["danmaku"]["font_size"])]
            )
        ...
        try:
            ...
            if sys.platform != "win32":
                ...
                try:
                    ...
                    if ffmpeg_process.returncode != 0:
                        ...
                        if (
                            ...
                            or "ubuntu" in os_release
                            or "debian" in os_release
                        ):
                        ...
                        raise HTTPException(
                            status_code=500, detail=f"FFmpeg 未安装\n\n{install_guide}"
                        )
                except Exception as e:
                    raise HTTPException(
                        status_code=500,
                ...
                try:
                    version_process = subprocess.run(
                        ["yutto", "--version"], capture_output=True, text=True
                    )
                    print(f"\nyutto 版本信息：")
            ...
            return StreamingResponse(
                stream_process_output(process), media_type="text/event-stream"
            )
        except Exception as e:
            # 记录详细的错误信息
            ...
            error_msg += (
                f"下载目录：{download_dir} (可写：{os.access(download_dir, os.W_OK)})\n"
            )
            ...
            try:
                ...
                if ffmpeg_process.returncode == 0:
                    ...
                    if version_process.returncode == 0:
                        error_msg += (
                            f"FFmpeg 版本：{version_process.stdout.splitlines()[0]}\n"
                        )
        ...
        raise HTTPException(
            status_code=500, detail="找不到 yutto 命令，请确保已正确安装"
        )
    except Exception as e:
...
async def check_ffmpeg():
    ...
    try:
        ...
        # 检查 FFmpeg 是否安装
        ffmpeg_process = subprocess.run(
            ffmpeg_check_cmd.split(), capture_output=True, text=True
        )
        ...
        if system == "darwin":  # macOS
            ...
            if os.path.exists("/etc/centos-release") or "centos" in os_release:
                ...
                install_guide += (
                    "未能识别的 Linux 发行版，请访问 FFmpeg 官网获取安装指南：\n"
                )
                install_guide += "https://ffmpeg.org/download.html"
...
async def check_video_download(cids: str):
    ...
    try:
        ...
        if not cid_list:
            return {"status": "error", "message": "未提供有效的 CID"}
                    ...
                    try:
                        ...
                        if not download_time and files:  # 确保有文件存在
                            ...
                            if os.path.exists(first_file_path):
                                ...
                                print(
                                    f"【调试】使用文件创建时间作为下载时间：{download_time}"
                                )
        ...
        return {"status": "error", "message": f"检查视频下载状态时出错：{str(e)}"}
    ...
    try:
        ...
        for root, dirs, files in os.walk(download_dir):
            ...
            if search_term and search_term.lower() not in dir_name.lower():
                ...
                file_match = False
                for file in files:
                    if search_term.lower() in file.lower() and file.endswith(
                        (".mp4", ".flv", ".m4a", ".mp3")
                    ):
                        file_match = True
                        break
            ...
            for file in files:
                ...
                if file.endswith((".mp4", ".flv", ".m4a", ".mp3")):
                    ...
                    if (
                        search_term
                        and search_term.lower() not in file.lower()
                        and search_term.lower() not in dir_name.lower()
                    ):
                        continue
                    ...
                    try:
                        ...
                        if not date_time:
                            ...
                            date_time = datetime.fromtimestamp(creation_time).strftime(
                                "%Y-%m-%d %H:%M:%S"
                            )
            ...
            if video_files:
                ...
                if metadata:
                    try:
                        ...
                        if _process_image_url:
                            # 使用导入的函数处理图片 URL
                            if video_info["cover"]:
                                video_info["cover"] = _process_image_url(
                                    video_info["cover"], "covers", use_local_images
                                )
                            if video_info["author_face"]:
                                video_info["author_face"] = _process_image_url(
                                    video_info["author_face"],
                                    "avatars",
                                    use_local_images,
                                )
                        elif hasattr(
                            sys.modules.get("routers.history"), "_process_image_url"
                        ):
                            ...
                            if video_info["cover"]:
                                video_info["cover"] = process_url(
                                    video_info["cover"], "covers", use_local_images
                                )
                            if video_info["author_face"]:
                                video_info["author_face"] = process_url(
                                    video_info["author_face"],
                                    "avatars",
                                    use_local_images,
                                )
                            ...
                            if video_info["cover"]:
                                cover_hash = hashlib.md5(
                                    video_info["cover"].encode()
                                ).hexdigest()
                                video_info["cover"] = (
                                    f"http://localhost:8899/images/local/covers/{cover_hash}"
                                )
                            if video_info["author_face"]:
                                avatar_hash = hashlib.md5(
                                    video_info["author_face"].encode()
                                ).hexdigest()
                                video_info["author_face"] = (
                                    f"http://localhost:8899/images/local/avatars/{avatar_hash}"
                                )

                        print(
                            f"【调试】从元数据获取到视频信息：{video_info['title']}，封面 URL: {video_info['cover'][:50]}..."
                        )
                    except Exception as e:
                ...
                if nfo_data and (
                    not video_info["cover"]
                    or not video_info["author_name"]
                    or not video_info["author_face"]
                ):
                    try:
                        # 提取标题
                        ...
                        if (
                            thumb_elem is not None
                            and thumb_elem.text
                            and not video_info["cover"]
                        ):
                        ...
                        if actor_elem is not None:
                            ...
                            if (
                                ...
                                and actor_name.text
                                and not video_info["author_name"]
                            ):
                            ...
                            if (
                                actor_thumb is not None
                                and actor_thumb.text
                                and not video_info["author_face"]
                            ):
                            ...
                            if (
                                ...
                                and actor_profile.text
                                and not video_info["author_mid"]
                            ):
                                profile_url = actor_profile.text
                                # 尝试从 URL 中提取 mid
                                mid_match = re.search(
                                    r"space\.bilibili\.com/(\d+)", profile_url
                                )
                                if mid_match:
                        ...
                        if (
                            website_elem is not None
                            and website_elem.text
                            and not video_info["bvid"]
                        ):
                        ...
                        print(
                            f"【调试】从 NFO 文件获取到视频信息：{video_info['title']}，封面 URL: {video_info['cover'][:50] if video_info['cover'] else 'None'}"
                        )
                    except Exception as e:
                ...
                if (
                    ...
                    try:
                        ...
                        print(
                            f"【调试】没有找到元数据或 NFO 文件，尝试通过 API/数据库获取 CID={cid}的视频信息"
                        )

                        # 方式 1: 直接调用 get_video_by_cid 函数（如果已成功导入）
                        if get_video_by_cid:
                            print(
                                f"【调试】使用导入的 get_video_by_cid 函数获取 CID={cid}的视频信息"
                            )
                            # 调用 API 函数获取视频信息
                            api_response = await get_video_by_cid(
                                int(cid), use_local_images
                            )
                            ...
                            if (
                                ...
                                video_info["title"] = (
                                    video_data.get("title") or video_info["title"]
                                )
                                video_info["cover"] = video_data.get("cover")
                                video_info["author_face"] = video_data.get(
                                    "author_face"
                                )
                                video_info["author_name"] = video_data.get(
                                    "author_name"
                                )
                                video_info["author_mid"] = video_data.get("author_mid")
                                video_info["bvid"] = video_data.get(
                                    "bvid"
                                )  # 添加 bvid 字段
                                print(
                                    f"【调试】成功通过 API 获取到视频信息：{video_data.get('title')}"
                                )
                        # 方式 2: 如果 API 函数未导入，则回退到直接查询数据库
                        elif db_available:
                            print(
                                f"【调试】回退到直接查询数据库获取 CID={cid}的视频信息"
                            )
                            cursor = conn.cursor()
                            # 查询所有历史记录表
                            years = [
                            ...
                            if years:
                                ...
                                for year in years:
                                    queries.append(
                                        f"SELECT title, cover, author_face, author_name, author_mid, bvid FROM bilibili_history_{year} WHERE cid = {cid} LIMIT 1"
                                    )
                                ...
                                if result:
                                    # 设置封面和作者信息
                                    video_info["title"] = (
                                        result["title"] or video_info["title"]
                                    )
                                    ...
                                    if _process_image_url:
                                        ...
                                        sys.modules.get("routers.history"),
                                        "_process_image_url",
                                    ):
                                        # 如果导入失败但模块运行时可访问，再次尝试
                                        process_url = getattr(
        ...
        # 根据修改时间排序，最新的在前面
        videos.sort(
            key=lambda x: max([f["modified_time"] for f in x["files"]])
            if x["files"]
            else 0,
            reverse=True,
        )
        ...
        return {
            "status": "success",
            "message": f"找到{total_videos}个视频"
            + (f"，匹配'{search_term}'" if search_term else ""),
        ...
        return {"status": "error", "message": f"获取已下载视频列表时出错：{str(e)}"}
...
async def stream_video(file_path: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取视频流时出错：{str(e)}")
...
async def delete_downloaded_video(
    ...
    try:
        ...
        # 确保下载目录存在
        if not os.path.exists(download_dir):
            return {"status": "error", "message": "下载目录不存在"}
        ...
        if directory and os.path.exists(directory):
            ...
            if delete_directory:
                ...
                for file in os.listdir(directory):
                    ...
                    if (
                        cid is None or f"_{cid}" in file
                    ):  # 如果指定了 CID 则检查文件名是否包含它
                        found_files.append({"file_name": file, "file_path": file_path})
                        ...
                        for file in files:
                            ...
                            if f"_{cid}" in file:
                                ...
                                if file.endswith((".mp4", ".flv", ".m4a", ".mp3")):
                                    file_path = os.path.join(root, file)
                                    found_files.append(
                                        {"file_name": file, "file_path": file_path}
                                    )
        ...
        return {"status": "error", "message": f"删除视频文件时出错：{str(e)}"}
...
async def stream_danmaku(file_path: Optional[str] = None, cid: Optional[int] = None):
    ...
    try:
        ...
        if file_path:
            ...
            if not os.path.exists(file_path):
                raise HTTPException(
                    status_code=404, detail=f"视频文件不存在：{file_path}"
                )
        ...
        # 检查是否找到弹幕文件
        if not danmaku_path:
            raise HTTPException(status_code=404, detail=f"未找到匹配的弹幕文件")
        ...
        raise HTTPException(status_code=500, detail=f"获取弹幕文件时出错：{str(e)}")
...
async def download_user_videos(request: UserSpaceDownloadRequest):
    ...
    try:
        ...
        command = [
            ...
            tmp_dir,
            "--subpath-template",
            f"{{username}}的全部投稿视频/{{title}}_{{download_date@%Y%m%d_%H%M%S}}/{{title}}",
            "--with-metadata",  # 添加元数据文件保存
        ]
        ...
        # 执行命令
        print(
            f"执行下载命令：{' '.join(command) if sys.platform == 'win32' else command_str}"
        )
...
async def download_favorites(request: FavoriteDownloadRequest):
    ...
    try:
        ...
        if not sessdata:
            raise HTTPException(
                status_code=401, detail="未登录：下载收藏夹必须提供 SESSDATA"
            )
        ...
        command = [
            ...
            tmp_dir,
            "--subpath-template",
            f"{{username}}的收藏夹/{{title}}_{{download_date@%Y%m%d_%H%M%S}}/{{title}}",
            "--with-metadata",  # 添加元数据文件保存
        ]
    ...
    try:
        ...
        if not aid and not bvid:
            return VideoDetailResponse(
                status="error", message="必须提供 aid 或 bvid 参数中的至少一个"
            )
        ...
        # 返回成功响应
        return VideoDetailResponse(
            status="success", message="获取视频信息成功", data=response_json.get("data")
        )
